...
object BedrockBridgeL2Extractor extends BridgeExtractor {
  ...
  override def extract(transactions: Array[Transaction]): ListBuffer[Array[PostgresqlRow]] = {
    val l2ToL1TransactionDeposit = transactions.filter(_.receipt.logs.exists(_.topic0 == sentMessageEvent.signature))
      .map(parseL1withdrawalOnL2)
  ...
  def parseL1withdrawalOnL2(transaction: Transaction): WithdrawalOnL2 = {
